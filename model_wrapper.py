...
class InterventionSettings:
    ...
    def __str__(self):
        ...
        if hasattr(self, "skip_last_layer"):
            int_name += "_skip_last" if self.skip_last_layer else ""
        return int_name
...
def main_hook_fn(module, inputs):
    ...
    if LOG_DEBUG:
        if random.random() < 0.01:
            print(
                f"running main hook on module: {module.__class__.__name__}, user_mask_sum={module.data_ref.user_mask.sum().item()}, assistant_mask_sum={module.data_ref.assistant_mask.sum().item()}"
            )
...
class ModelWrapper(torch.nn.Module):
    ...
    def set_intervention(self, settings: InterventionSettings):
        ...
        blocks = self.blocks
        self.hooks.append(embedding.register_forward_pre_hook(main_hook_fn))
    ...
    def get_role_nlls(self, tokens: torch.Tensor):
        ...
        tokens = tokens[0].cpu()[
            1:
        ]  # remove SOS token which doesn't have an associated NLL
        results = []
        current_role = None
        current_nll_sum = 0
        role_started = False
